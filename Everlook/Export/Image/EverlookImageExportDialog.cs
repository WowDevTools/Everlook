...
namespace Everlook.Export.Image
{
	...
	public partial class EverlookImageExportDialog : Dialog
	{
		...
		{
			string ImageFilename = System.IO.Path.GetFileNameWithoutExtension(Utilities.ConvertPathSeparatorsToCurrentNativeSeparator(ExportTarget.ItemPath));
		...
		public void RunExport()
		{
			...
			{
				ExportPath =
					$"{ExportDirectoryFileChooserButton.Filename}{System.IO.Path.DirectorySeparatorChar}{Utilities.ConvertPathSeparatorsToCurrentNativeSeparator(ExportTarget.ItemPath).Replace(".blp", "")}";
			}
			else
			{
